...
mapboxgl.accessToken =
   "pk.eyJ1Ijoiam9hb3NjaGVpZDExMTIiLCJhIjoiY204ZGdxbGRwMHAycjJsb3dvY2NxdjZybSJ9.LBq6pnASGALoZor7X3zcbQ";
      ...
      try {
         ...
         const mainMarker = new mapboxgl.Marker({ color: "#FF0000" })
          ...
          <div style="background-color: #6B1D1D; padding: 12px;">
            ...
            <p style="margin: 4px 0; color: #F5F5DC !important;">${
               endereco.rua
            }, ${endereco.numeroCasaPredio}</p>
            ${
               endereco.numeroCasaPredio
                  ? `<p style="margin: 4px 0; color: #F5F5DC !important;">Apto ${endereco.numeroCasaPredio}</p>`
                  : ""
            }
            <p style="margin: 4px 0; color: #F5F5DC !important;">${
               endereco.bairro
        ...
        `)
         ...
         const locais = await getLocaisProximosServer(
            coords.latitude,
            coords.longitude
         );
         ...
         if (locais.length > 0) {
            ...
            locais.forEach((local, index) => {
               ...
               const marker = new mapboxgl.Marker({ color: "#4A90E2" })
                ...
                <div style="background-color: #6B1D1D; padding: 12px;">
                  <h3 style="font-weight: bold; font-size: 1.125rem; margin-bottom: 4px; color: #F5F5DC !important;">${
                     local.nome
                  }</h3>
                  <p style="margin: 4px 0; color: rgba(245, 245, 220, 0.8) !important;">${
                     local.tipo
                  }</p>
                  <p style="font-size: 0.875rem; margin-top: 8px; color: #F5F5DC !important;">${
                     local.endereco
                  }</p>
