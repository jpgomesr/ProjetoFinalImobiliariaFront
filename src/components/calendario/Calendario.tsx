      ...
      <div className="flex flex-col h-full">
         ...
         <div className="grid grid-cols-7 text-center text-xs">
            {weekDays.map((day) => (
               <div
                  key={day}
                  className="py-2 font-medium flex items-center justify-center w-8 h-8 mx-auto text-begepadrao"
               >
                  {day}
               </div>
            ))}
         ...
         {/* Dias do mês */}
         <div className="grid grid-cols-7 text-center gap-1 flex-grow pb-4">
            {days.map((day, index) => {
               ...
               return (
                  <BotaoCalendario
                     ...
                     className={twMerge(
                        ...
                        isToday &&
                           !isSelected &&
                           "border border-begepadrao text-begepadrao rounded-full", // Dia atual
                        isSelected &&
                           "bg-white text-havprincipal rounded-full font-medium", // Dia selecionado
                        !isToday &&
                           !isSelected &&
                           isCurrentMonth &&
                           !isPastDay &&
                           "hover:bg-havprincipal text-begepadrao", // Dias normais do mês atual
                        isLastRow && "mb-1",
