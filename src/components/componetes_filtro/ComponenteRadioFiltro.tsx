...
const ComponenteRadioFiltro = (props : ComponenteRadioFiltroProps) => {
    const [selecionados, setSelecionados] = useState<number[]>(props.selecionados);
      ...
      <div className="flex flex-col text-xs gap-1">
        ...
        <div className="flex text-[10px] gap-2">
          {opcoes.map((opcao) => (
            <div
              key={opcao}
              className={`w-5 h-5 flex items-center justify-center rounded-full cursor-pointer ${props.selecionados.includes(opcao) ? 'bg-havprincipal text-white' : 'bg-begepadrao'}`}
