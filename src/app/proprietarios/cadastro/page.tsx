    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (responseData.erros) {
          ...
          const primeiroCampoComErro = Object.keys(errosFormatados)[0] as keyof validatorSchema
      ...
      router.push("/proprietarios?cadastroSucesso=true")
    } catch (error) {
        ...
        >
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex flex-col">
              ...
              />
              {errors.tipoResidencia && <span className="text-red-500 text-sm">{errors.tipoResidencia.message}</span>}
            </div>
            ...
            <Controller
              name="imagemPerfil"
              control={control}
              render={({ field }) => <UploadImagem onChange={(file: File | null) => field.onChange(file)} />}
            />
