...
interface PerfilClientProps {
  initialData: {
    ...
    foto: string;
    role: string;
    metodo2FA: "email" | "sms" | null;
  };
}

export default function PerfilClient({ initialData }: PerfilClientProps) {
  const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL;
  const router = useRouter();
  const { showNotification } = useNotification();
  const [preview, setPreview] = useState<string>(initialData.foto || "");
  const [loading, setLoading] = useState(false);
  const [isEditingImage, setIsEditingImage] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [selectedContact, setSelectedContact] = useState(initialData.email ? "email" : "telefone");
  const [selected2FA, setSelected2FA] = useState<"email" | "sms" | null>(initialData.metodo2FA);
  ...
  const onSubmit = async (data: usuarioValidatorSchema) => {
    try {
      const response = await UseFetchPostFormData(
        `${BASE_URL}/usuarios/${initialData.id}`,
        {
            ...
            <div className="flex flex-col xl:flex-row items-center gap-1 sm:gap-2 md:gap-3 xl:gap-4">
              ...
              <div className="flex flex-col">
                ...
                <p className="w-32 sm:w-40 md:w-48 xl:w-[600px] text-xs sm:text-sm xl:text-start text-gray-500 text-center px-1 sm:px-2">
                  Use o código de segurança enviado para o seu e-mail como a sua autenticação de dois fatores (2FA). O código de segurança será enviado ao e-mail vinculado à sua conta
                </p>
              </div>
            </div>
            <button
            ...
            <div className="flex flex-col xl:flex-row items-center gap-1 sm:gap-2 md:gap-3 xl:gap-4">
              ...
              <div className="flex flex-col">
                ...
                <p className="w-32 sm:w-40 md:w-48 xl:w-[650px] text-xs sm:text-sm xl:text-start text-gray-500 text-center px-1 sm:px-2">
                  Use o seu número de telefone como o seu código de autenticação de dois fatores (2FA).Você precisará fornecer o código de segurança que o enviamos via mensagem SMS
                </p>
              </div>
            </div>
            <button
