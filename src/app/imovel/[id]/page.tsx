...
import { BedDouble, Car, Heart, Ruler, Share2, ShowerHead, WavesLadder, ChevronLeft, ChevronRight } from "lucide-react";
...
const Page = () => {
  ...
  const [imoveisSemelhantes, setImoveisSemelhantes] = useState<ImovelSemelhante[]>([]);
  ...
  const buscarImoveisSemelhantes = async () => {
    try {
      ...
      const imoveis = Array.isArray(response.imoveis) ? response.imoveis : [];
      console.log("Imóveis para filtrar:", imoveis); // Debug array de imóveis
      
      if (!imovel) return; // Garantir que temos o imóvel atual

      const semelhantes = imoveis.filter((imovelComparado: ImovelSemelhante) => {
        ...
        if (!imovelComparado || typeof imovelComparado.preco !== 'number' || !imovelComparado.qtdBanheiros || !imovelComparado.qtdQuartos) {
        ...
        try {
          ...
          console.log("Comparações:", {
            banheiros: imovelComparado.qtdBanheiros === imovel.qtdBanheiros,
    ...
    <Layout className="bg-begepadrao py-8">
      <div className="flex flex-col items-center w-full gap-1 md:flex-row md:px-8 md:items-start">
        ...
        <div className="flex w-11/12 flex-col gap-1 items-center md:items-start lg:ml-24 md:ml-20">
          <div className="flex justify-center items-center w-full relative">
            ...
            <Image
              src={imovel.imagens[currentImageIndex]?.referencia || IMAGEM_PADRAO}
              alt="House image"
              width={1920}
              height={1080}
            ...
            {[...imovel.imagens, ...imovel.imagens] // Duplicamos o array para permitir rotação circular
              ...
              .map((imagem, index) => (
                <Image
                  ...
                  onClick={() => {
                    const targetIndex = (currentImageIndex + 1 + index) % imovel.imagens.length;
        ...
        <div className="w-9/12 font-inter text-havprincipal mt-3 md:mt-0 md:ml-24 lg:ml-0">
          ...
          {imovel.precoPromocional ? (
            <>
              <h1 className="font-extrabold text-xl text-shadow md:text-3xl">R${imovel.precoPromocional}</h1>
              <p className="text-sm line-through opacity-75">R${imovel.preco}</p>
            </>
          ) : (
          ...
          <p className="w-full text-start md:w-9/12 md:text-lg">{imovel.descricao}</p>
          {imovel.iptu && (
            <p className="mt-2">
      ...
      {imoveisSemelhantes.length > 0 && (
        <div className="mt-10 px-8">
          <h2 className="text-2xl font-semibold text-havprincipal mb-6">Imóveis Semelhantes</h2>
